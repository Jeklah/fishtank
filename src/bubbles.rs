...
let init_vel = SetAttributeModifier {
    arrtibute: Attribute::VELOCITY,
    value: WriterExpr::vec3(writer.lit(0.0), writer.lit(0.2), writer.lit(0.0)).expr(),
};

let mut color_gradient = Gradient::new();
color_gradient.add_key(0.0, Vec4::new(.9, .9, 1.0, .8));
color_gradient.add_key(.7, Vec4::new(.8, .8, 1.0, .7));
color_gradient.add_key(1.0, Vec4::new(.8, .8, 1.0, .0));
...
size_gradient.add_key(.3, Vec2::splat(0.05));
