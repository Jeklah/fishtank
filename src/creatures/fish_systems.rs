...
use super::fishtank::{FishMortality, FishSkeleton};
...
fn setup_fish_system(
    ...
    let fish_mesh = asset_server.load(
        ...
        .from_asset("embedded://fishtank/../assets/fish.glb"),
    );
    ...
    for spawn_location in spawn_events.read() {
        ...
        commands.spawn((
            ...
            PbrBundle {
                ...
                material: fish_materials.choose(&mut rng).unwrap().clone(),
                ..default()
            },
        ));
    }
...
fn fish_pellet_detection_system(
    ...
    for (mut transform, mut behavior, mut mortality) in fishes.iter_mut() {
        FishOperations::new(&mut transform, &mut behavior, &mut mortality).detect_pellets(&pellets);
    }
}
